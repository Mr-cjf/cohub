...
DSH 移植版已拆分为独立仓库：`packages/dsh-cohub`（GitHub 待建：`Mr-cjf/dsh-cohub`）。12 个 co-* 代理以 runtime skills 注册 + 中文指令 section + council 共识工具 + co-orchestrator preset。

- 构建：`cd packages/dsh-cohub && npm run build`（prepublishOnly 自动构建）
